...
float rotBuzzHead=0.0, rotBuzzLeftarm=0.0, rotBuzzLeftForeArm=0.0, rotBuzzLeftHand=0.0, rotBuzzLeftThigh=0.0, rotBuzzRightArm=0.0, rotBuzzRightForeArm=0.0,rotBuzzRightHand=0.0, rotBuzzRightLeg=0.0,rotBuzzLeftLeg=0.0;
int modelSelected = 0;
bool enableCountSelected = true;
float avance=0.1, giroEclipse=0.5, giroLamb=0.5, detener=0.0;
float avanceL=0.1;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	modelBuzzLeftFood.loadModel("../models/buzz/buzzlightyLeftFoot.obj");
	modelBuzzLeftFood.setShader(&shaderMulLighting);
...
void applicationLoop() {
	...
	int state = 0;
	int stateL=0;
	float advanceCount = 0.0, advanceCountL = 0.0;
	float rotCount = 0.0, rotCountL=0.0;
	float rotWheelsX = 0.0;
	float rotWheelsY = 0.0;
	float rotWheelsLY=0.0;
	float rotWheelsLX=0.0;
	int numberAdvance = 0, numberAdvanceL=0;
	int maxAdvance = 0.0, maxAdvanceL=0.0;
	...
	keyFramesBuzzJoints = getKeyRotFrames(fileName);
	keyFramesBuzz = getKeyFrames("../animaciones/animation_buzz_joints.txt");
	...
	while (psi) {
		...
		shaderMulLighting.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		//Settea la matriz de vista y projection al shader 
		...
		*********************************************/
		...
		shaderMultiplesTexturas.setInt("texture1", 5);
		glActiveTexture(GL_TEXTURE10);
		glBindTexture(GL_TEXTURE_2D,textureSpongeID);
		shaderMultiplesTexturas.setInt("texture2",10);
		...
		*******************************************/
		...
		modelMatrixHip= glm::translate(modelMatrixHip, glm::vec3(0,0,0.488899));
		modelMatrixHip= glm::translate(modelMatrixHip, glm::vec3(0,0,-0.488899));
		modelBuzzHip.render(modelMatrixHip);

		glm::mat4 modelMatrixLeftThigh=glm::mat4(modelMatrixHip);
		modelMatrixLeftThigh= glm::translate(modelMatrixLeftThigh,glm::vec3(0.062714 , 0.366638, -0.001494 ));
		modelMatrixLeftThigh= glm::rotate(modelMatrixLeftThigh, rotBuzzLeftLeg, glm::vec3(0,0,1) );
		modelMatrixLeftThigh= glm::translate(modelMatrixLeftThigh, glm::vec3(-0.062714 ,-0.366638, 0.001494));
		modelBuzzLeftThigh.render(modelMatrixLeftThigh);

		glm::mat4 modelMatrixLeftCalf=glm::mat4(modelMatrixLeftThigh);
		modelMatrixLeftCalf= glm::translate(modelMatrixLeftCalf,glm::vec3(0.062688  , 0.238395 , -0.058382 ));
		modelMatrixLeftCalf= glm::rotate(modelMatrixLeftCalf, rotBuzzLeftLeg, glm::vec3(0,0,1) );
		modelMatrixLeftCalf= glm::translate(modelMatrixLeftCalf, glm::vec3(-0.062688  ,-0.238395 , 0.058382));
		modelBuzzLeftCalf.render(modelMatrixLeftCalf);

		glm::mat4 modelMatrixLeftFoot=glm::mat4(modelMatrixLeftCalf);
		modelMatrixLeftFoot= glm::translate(modelMatrixLeftFoot,glm::vec3(0.070712 , 0.106768 , -0.004202  ));
		modelMatrixLeftFoot= glm::rotate(modelMatrixLeftFoot, rotBuzzLeftLeg, glm::vec3(0,0,1) );
		modelMatrixLeftFoot= glm::translate(modelMatrixLeftFoot, glm::vec3(-0.070712  ,-0.106768 , 0.004202 ));
		modelBuzzLeftFood.render(modelMatrixLeftFoot);

		glm::mat4 modelMatrixRightArm=glm::mat4(modelMatrixTorso);
		modelMatrixRightArm= glm::translate(modelMatrixRightArm,glm::vec3(-0.164357  , 0.577592, -0.022103));
		...
		modelMatrixRightThigh= glm::translate(modelMatrixRightThigh,glm::vec3(-0.062714 , 0.366638, -0.001494 ));
		modelMatrixRightThigh= glm::rotate(modelMatrixRightThigh, rotBuzzRightLeg, glm::vec3(0,0,1) );
		modelMatrixRightThigh= glm::translate(modelMatrixRightThigh, glm::vec3(0.062714 ,-0.366638, 0.001494));
		modelBuzzRightThigh.render(modelMatrixRightThigh);

		glm::mat4 modelMatrixRightCalf=glm::mat4(modelMatrixRightThigh);
		modelMatrixRightCalf= glm::translate(modelMatrixRightCalf,glm::vec3(-0.062688  , 0.238395 , -0.058382 ));
		modelMatrixRightCalf= glm::rotate(modelMatrixRightCalf, rotBuzzRightLeg, glm::vec3(0,0,1) );
		modelMatrixRightCalf= glm::translate(modelMatrixRightCalf, glm::vec3(0.062688  ,-0.238395 , 0.058382));
		modelBuzzRightCalf.render(modelMatrixRightCalf);

		glm::mat4 modelMatrixRightFoot=glm::mat4(modelMatrixRightCalf);
		modelMatrixRightFoot= glm::translate(modelMatrixRightFoot,glm::vec3(-0.070712 , 0.106768 , -0.004202  ));
		modelMatrixRightFoot= glm::rotate(modelMatrixRightFoot, rotBuzzRightLeg, glm::vec3(0,0,1) );
		modelMatrixRightFoot= glm::translate(modelMatrixRightFoot, glm::vec3(0.070712  ,-0.106768 , 0.004202 ));
		modelBuzzRightFoot.render(modelMatrixRightFoot);
		glm::mat4 modelMatrixLeftWing1=glm::mat4(modelMatrixTorso);
		modelMatrixLeftWing1= glm::translate(modelMatrixLeftWing1,glm::vec3(0.0 , 0.0    ,0.01666     ));
		modelMatrixLeftFoot= glm::translate(modelMatrixLeftWing1, glm::vec3(0.0  , 0.0   ,-0.01666  ));
		modelBuzzleftWing1.render(modelMatrixLeftWing1);

		glm::mat4 modelMatrixLeftWing2=glm::mat4(modelMatrixLeftWing1);
		modelMatrixLeftWing2= glm::translate(modelMatrixLeftWing2,glm::vec3(0.0  ,0.0   , 0.0  ));
		modelMatrixLeftWing2= glm::translate(modelMatrixLeftWing2, glm::vec3(0.0 , 0.0    , 0.0 ));
		modelBuzzleftWing2.render(modelMatrixLeftWing2);

		glm::mat4 modelMatrixRightWing1=glm::mat4(modelMatrixTorso);
		modelMatrixRightWing1= glm::translate(modelMatrixRightWing1,glm::vec3(0.0 , 0.0    ,0.01666     ));
		modelMatrixRightFoot= glm::translate(modelMatrixRightWing1, glm::vec3(0.0  , 0.0   ,-0.01666  ));
		modelBuzzRightWing1.render(modelMatrixRightWing1);

		glm::mat4 modelMatrixRightWing2=glm::mat4(modelMatrixRightWing1);
		modelMatrixRightWing2= glm::translate(modelMatrixRightWing2,glm::vec3(0.0  ,0.0   , 0.0  ));
		modelMatrixRightWing2= glm::translate(modelMatrixRightWing2, glm::vec3(0.0 , 0.0    , 0.0 ));
		...
		case 1: 
			...
			glm::vec3(0,0,avance));
			advanceCount+=avance;
			rotWheelsX+=0.05; //las llanatas giren cuando avance
			rotWheelsY-=0.02; //es negativo por el sentido
			if(rotWheelsY<=0){
				rotWheelsY=0;
			} //se reinicia el valor cuando gira 
		...
		case 2: 
			...
			rotWheelsY+=0.02;
			if(rotWheelsY >=0.25){
				rotWheelsY=0.25;  //cuando llegue a un maximo no se tuerza la llanta 
			}
			if(rotCount>=90.0f){
				rotCount=0;
				state=0; //finalice girando
				if(numberAdvance>4)
				numberAdvance=1;
			}
		break;
