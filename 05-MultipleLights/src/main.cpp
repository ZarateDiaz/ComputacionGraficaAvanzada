...
std::vector<float>lamp1Orientatio={
	-17.0,-82.67,23.70
};
...
void applicationLoop() {
	...
	while (psi) {
		 ...
		 *******************************************/
			...
			shaderMulLighting.setInt("spointLightCount", 1);
		shaderTerrain.setInt("spointLightCount", 1);
		...
		glm::vec3 SpotPosition=glm::vec3(modelMatrixHeli* glm::vec4(0.0,0.2,1.75,1.0));
			...
			shaderMulLighting.setVectorFloat3("spotLights[0].light.difuse",
			...
			glm::value_ptr(glm::vec3(0.0,0.0,0.0)));
			shaderTerrain.setVectorFloat3("spotLights[0].light.difuse",
...
*/
		...
		for (int i=0; i<lamp1Position.size();i++){
			...
			matAux=glm::rotate(matAux, glm::radians(lamp1Orientatio[i]), glm::vec3(0,1,0));
		...
		for (int i=lamp1Position.size(); i<lamp1Position.size()+lamp2Position.size();i++){
			glm::mat4 matAux=glm::mat4(1.0);
			matAux=glm::translate(matAux, lamp2Position[i-lamp1Position.size()]);
			matAux=glm::rotate(matAux, glm::radians(lamp2Orientation[i-lamp1Position.size()]),glm::vec3(0,1,0)); 
			matAux=glm::scale(matAux, glm::vec3(1.0));
			matAux=glm::translate(matAux, glm::vec3(0.77,5.021,0));
			...
			glm::value_ptr(glm::vec3(0.4,0.32,0.02)));
			shaderTerrain.setVectorFloat3("pointLights["+ std::to_string(i) +"].light.specular",
			glm::value_ptr(glm::vec3(0.6, 0.,0.03)));
			shaderTerrain.setVectorFloat3("pointLights["+ std::to_string(i) +"].position",
			glm::value_ptr(positionLamp));
			shaderTerrain.setFloat("pointLights["+ std::to_string(i) +"].constant",
			1.0);
		...
		for (unsigned int i=0; i< lamp1Position.size(); i++){
			...
			modelLamp1.setOrientation(glm::vec3(0,lamp1Orientatio[i],0));
