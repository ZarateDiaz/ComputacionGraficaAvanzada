...
float distanceFromPlayer = 9.0;
float angleTarget = -90.0;
float angleTarget2 = 180.0;
...
void applicationLoop() {
	...
	while (psi) {
		...
		glm::mat4 view; 
		//alternativa en camaras 
		
		if(CPP){
			glm::vec3 forwardDirection = glm::normalize(glm::vec3(modelMatrixAmongUs[2]));
			glm::vec3 headPosition = positionTarget + glm::vec3(0.0f, modelMatrixAmongUs[3][1], 0.0f); 
			FP_camera->setPosition(headPosition+forwardDirection);
